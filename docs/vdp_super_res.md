...
This command is similar to `LMMM` (Logical VRAM to VRAM), but instead of using a data source of a bounded rectangle (`SX`, `SY`), the `BMXL` command retrieves its source data from a linear address space (`SA`) within the VRAM.  The registers used to supply the parameters for the command are the same as `LMMM`, expect that `SX` and `SY` is replaced with `SA` (Source Address).
...
| #R46[3:0] | `LO`                | Logical Operator to be applied (see above)                      |
